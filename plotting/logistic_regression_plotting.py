...
mdl = load(os.path.join(config.MODEL_DIR, 'svc_simple.mdl'))
...
chemistry_results = chemistry_results[['ALP', 'ALT', 'AST', 'BILI']]
...
data = data[data[['ALP', 'ALT', 'AST', 'BILI']].notnull().all(1)]
...
X = data[['ALP', 'ALT', 'AST', 'BILI', 'SPECIES', 'SEX']]
...
plt.savefig(os.path.join(config.IMG_DIR, 'edish_covered_{}.png'.format(sex)), transparent=True)



#

xx, yy = np.mgrid[0:1:0.01,
                  0:1:0.01]

#
fake_data = np.c_[np.full(xx.ravel().shape, X.ALP.mean()), xx.ravel(),
                  np.full(xx.ravel().shape, X.AST.mean()), yy.ravel(),
...
xx = np.log(xx+abs(alt_min)+1)
yy = np.log(yy+abs(bili_min)+1)
