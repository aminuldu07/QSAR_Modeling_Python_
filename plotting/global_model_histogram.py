...
fig, axarr = plt.subplots(nrows=1, ncols=2, figsize=plt.figaspect(1/2), sharex=True)
...
N, bins, patches = axarr[0].hist(final_data.PREDICTION_MEAN[final_data[d_name] == 0], zorder=2, bins=bin_defs)
...
N, bins, patches = axarr[1].hist(final_data.PREDICTION_MEAN[final_data[d_name] == 1], zorder=2, bins=bin_defs)
for b, thispatch in zip(bins, patches):
    color = plt.cm.seismic(b)
    thispatch.set_facecolor(color)
    thispatch.set_edgecolor('black')
...
axarr[0].set_title('{} Negative'.format(d_name.title()))
axarr[1].set_title('{} Positive'.format(d_name.title()))
...
for idx in [0, 1]:
    ...
    axarr[idx].set_xticks([0, 0.5, 1])
    axarr[idx].set_xticklabels([x for x in axarr[idx].get_xticks()], fontsize=16)
    axarr[idx].set_yticklabels([int(y) for y in axarr[idx].get_yticks()], fontsize=16)
    # axarr[idx].set_xticks(0, 1)
# axarr[i].suptitle(s)

plt.subplots_adjust(wspace=0.3, hspace=0.5)

plt.savefig(os.path.join(config.IMG_DIR, 'average_model_preds_DD.png'), transparent=True)
